...
class StatsScreen (private var characterScreen: CharacterScreen) {
    ...
    init{
        System.out.println("INIT STATSCREEN" + statsScreen);
        ...
        statsScreen.setOnCancelListener {
            ...
            character.rewardObtained =
                statsScreen.stats_reward_obtained.text.toString().equals("Yes")
    ...
    open fun showDialog()
    {
        System.out.println("DIALOG" + statsScreen)
        var level = 5
    ...
    lateinit var currentStatEditing: TextView
    ...
    fun setTotalKills() {
        ...
        if (character.timesWounded > 0) {
            statsScreen.stats_kill_death_ratio.text =  (Math.round(10*totalKills.toFloat()
                    / character.timesWounded).toFloat()/10f).toString()
        } else {
