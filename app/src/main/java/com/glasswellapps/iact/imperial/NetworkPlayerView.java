...
public class NetworkPlayerView {
    ...
    public NetworkPlayerView(View view, Activity context){
        this.view = view;
    ...
    public void updateImages(Character character){
        CharacterImageController.Companion.update(character,characterImageView,companionImageView
                , context);
    }
    ...
    public void setConditions(boolean[] conditionsActive) {
        for(int i = 0; i < conditionsActive.length; i++){
            ...
            conditionViews[i].animate().setDuration(150).alpha(isActive?0.8f:0);
        }
    }
